...
from coach_app.app.db.models import DailyLog, SystemPrompt, UserProfile

DEFAULT_SYSTEM_PROMPT = (
    "You are a calm, supportive fitness and nutrition coach. You provide concise, structured guidance focusing on sustainable "
    "fat loss while maintaining muscle. Always consider menstrual cycle effects, under-fueling risks, and gym closures."
)
...
class PromptBuilder:
    ...
    def load_recent_logs(self, limit: int = 7) -> List[DailyLog]:
        return list(self.session.exec(select(DailyLog).order_by(DailyLog.log_date.desc()).limit(limit)))

    def build_messages(self, today_inputs: dict, today: Optional[date] = None) -> list:
        ...
        profile_block = {
            ...
            "menstrual_cycle_notes": profile.menstrual_cycle_notes
            if profile
            else "regular cycle impacts energy and water",
            "fueling_sensitivity": profile.fueling_sensitivity
            if profile
            else "sensitive to under-fueling and late-night hunger",
            "reassurance_needs": profile.reassurance_needs
            if profile
            else "wants reassurance and clear decision support",
        }
